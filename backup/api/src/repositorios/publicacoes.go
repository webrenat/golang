...
func (repositorio Publicacoes) Buscar(usuarioID uint64) ([]modelos.Publicacao, error) {
	...
	inner join usuarios u on u.id = p.autor_id inner join seguidores s on p.autor_id = s.usuario_id 
...
func (repositorio Publicacoes) Descurtir(publicacaoID uint64) error {
	statement, erro := repositorio.db.Prepare(`update publicacoes set curtidas = CASE WHEN curtidas > 0 curtidas - 1 ELSE curtidas END where id = ?`)
	if erro != nil {
		return erro
	}
